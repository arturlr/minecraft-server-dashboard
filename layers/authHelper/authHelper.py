...
class Auth:
    def __init__(self, cognito_pool_id):
        ...
        self.jwk_url = "https://cognito-idp.{}.amazonaws.com/{}/.well-known/jwks.json".format(
    ...
    def process_token(self, token):
        ...
        cog_user = self.cognito_idp.admin_get_user(UserPoolId=self.cognito_pool_id, Username=user_name)

        user_attributes["username"] = user_name

        for att in cog_user["UserAttributes"]:
            if att["Name"].lower() in [
                "email",
                "given_name",
                "family_name",
                "sub",
    ...
    def group_exists(self, instanceId):
        try:
            self.cognito_idp.get_group(GroupName=instanceId, UserPoolId=self.cognito_pool_id)
    ...
    def create_group(self, instanceId):
        try:
            self.cognito_idp.create_group(
                GroupName=instanceId, UserPoolId=self.cognito_pool_id, Description="Minecraft Dashboard"
            )
    ...
    def add_user_to_group(self, instance_id, userName):
        try:
            self.cognito_idp.admin_add_user_to_group(
                UserPoolId=self.cognito_pool_id, Username=userName, GroupName=instance_id
            )
    ...
    def list_users_for_group(self, instance_id):
        ...
        try:
            ...
            page_iterator = paginator.paginate(GroupName=instance_id, UserPoolId=self.cognito_pool_id)
    ...
    def find_user_by_email(self, email):
        ...
        try:
            ...
            response = self.cognito_idp.list_users(
                UserPoolId=self.cognito_pool_id, Filter=f'email = "{email}"', Limit=1
            )
            ...
            ErrorHandler.log_error(
                "INTERNAL_ERROR", context={"operation": "find_user_by_email", "email": email}, exception=e, error=str(e)
            )
            return None
    ...
    def get_user_by_sub(self, user_sub):
        ...
        try:
            ...
            response = self.cognito_idp.list_users(
                UserPoolId=self.cognito_pool_id, Filter=f'sub = "{user_sub}"', Limit=1
            )
        ...
        while True:
            if next_token:
                response = self.cognito_idp.admin_list_groups_for_user(
                    Username=username, UserPoolId=self.cognito_pool_id, NextToken=next_token
                )
            else:
                response = self.cognito_idp.admin_list_groups_for_user(
                    Username=username, UserPoolId=self.cognito_pool_id
                )
