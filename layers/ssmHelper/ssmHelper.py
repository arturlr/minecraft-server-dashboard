...
class SSMHelper:
    ...
    def queue_ssm_command(
        self, instance_id, document_name, parameters, comment=None, timeout_seconds=3600, metadata=None
    ):
        """
        ...
        if not self.queue_url:
            ...
            return {"success": False, "messageId": None, "message": "Queue URL not configured"}
        ...
        try:
            ...
            response = self.sqs.send_message(QueueUrl=self.queue_url, MessageBody=json.dumps(message_body))

            message_id = response["MessageId"]
            logger.info(f"Queued SSM command for {instance_id}: Document={document_name}, MessageId={message_id}")

            return {"success": True, "messageId": message_id, "message": "Command queued successfully"}

        except Exception as e:
            logger.error(f"Error queueing SSM command: {e}", exc_info=True)
            return {"success": False, "messageId": None, "message": str(e)}
    ...
    def queue_bootstrap_command(self, instance_id):
        ...
        if not self.bootstrap_doc_name:
            ...
            return {"success": False, "messageId": None, "message": "Bootstrap document name not configured"}
        ...
        return self.queue_ssm_command(
            ...
            metadata={"purpose": "bootstrap", "requestedBy": "system"},
        )
