...
class Utils:
    ...
    def get_metrics_data(self, instance_id, namespace, metric_name, unit, statistics, start_time, end_time, period=300):
        ...
        try:
            ...
            if len(datapoints) > 0:
                print(
                    f"Fetched {len(datapoints)} points for {metric_name}. First: {datapoints[0]}, Last: {datapoints[-1]}"
                )
            else:
    ...
    def check_user_authorization(
        self, user_sub_or_groups, server_id, required_permission_or_email="read_server", ec2_utils=None
    ):
        """
        ...
        try:
            ...
            if isinstance(user_sub_or_groups, list):
                ...
                logger.info(
                    f"Using legacy authorization check: user={user_email}, instance={instance_id}, groups={cognito_groups}"
                )
                ...
                required_permission = (
                    required_permission_or_email
                    if isinstance(required_permission_or_email, str)
                    and required_permission_or_email
                    in ["read_server", "read_metrics", "read_config", "control_server", "manage_users", "manage_config"]
                    else "read_server"
                )
                ...
                logger.info(
                    f"Using DynamoDB authorization check: user={user_sub}, server={server_id}, permission={required_permission}"
                )

                # Use the new DynamoDB-based permission checking
                try:
                    is_authorized, user_role, auth_reason = auth.check_user_permission(
                        user_sub, server_id, required_permission
                    )
                    ...
                    if is_authorized:
                        ...
                        logger.warning(
                            f"Authorization denied: user={user_sub}, server={server_id}, reason={auth_reason}"
                        )
            ...
            # Log the error
            ErrorHandler.log_error(
                "AUTHORIZATION_FAILED", context={"operation": "check_user_authorization"}, exception=e, reason=str(e)
            )
    ...
    def get_ssm_param(self, paramKey, isEncrypted=False):
        try:
            ssmResult = self.ssm.get_parameter(Name=paramKey, WithDecryption=isEncrypted)
    ...
    def get_ssm_parameters(self, paramKeys, isEncrypted=False):
        try:
            ...
            ssmResult = self.ssm.get_parameters(Names=paramArray, WithDecryption=isEncrypted)
            if ssmResult["Parameters"]:
                for entry in ssmResult["Parameters"]:
                    resp.append({"Name": entry["Name"], "Value": entry["Value"]})
    ...
    def put_ssm_param(self, paramKey, paramValue, paramType):
        try:
            ssmResult = self.ssm.put_parameter(Name=paramKey, Value=paramValue, Type=paramType, Overwrite=True)
    ...
    def retrieve_extension_value(self, url: str) -> Optional[Dict[Any, Any]]:
        ...
        try:
            ...
            # Make the request
            response = requests.get(url=full_url, headers=headers, timeout=30)  # Add timeout to prevent hanging
    ...
    def send_server_notification_email(
        self, recipient_email: str, server_name: str, action: str, instance_id: str, sender_email: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            if not sender_email:
                ...
                if not sender_email:
                    logger.error(
                        "No sender email configured in SSM parameter '/minecraft-dashboard/notification-sender-email'"
                    )
                    return False

            # Prepare email content based on action
            action_past_tense = {"start": "started", "stop": "stopped", "restart": "restarted"}.get(
                action.lower(), action
            )
...
This is an automated notification from your Minecraft Server Dashboard.
            ...
            if response["ResponseMetadata"]["HTTPStatusCode"] == 200:
                logger.info(
                    f"Email notification sent successfully to {recipient_email} for server {server_name} ({action_past_tense})"
                )
                return True
            else:
