...
class TestUserMembershipDyn(unittest.TestCase):
    ...
    def test_get_membership_success(self):
        ...
        self.mock_table.get_item.assert_called_once_with(
            Key={'userId': self.user_id, 'serverId': self.server_id}
        )
    ...
    def test_check_user_permission_invalid_permission(self):
        ...
        # Act & Assert
        with self.assertRaises(ValueError) as context:
            self.membership_dyn.check_user_permission(
                self.user_id, self.server_id, 'invalid_permission'
            )
        
        self.assertIn('Invalid permission', str(context.exception))

    @given(
        user_id=st.text(min_size=1, max_size=100, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'Pc'), whitelist_characters='-_')),
    ...
    def test_property_membership_data_integrity(self, user_id, server_id, email, role, created_by):
        ...
        created_membership = self.membership_dyn.create_membership(
            user_id, server_id, email, role, created_by
        )
        ...
        self.mock_table.get_item.assert_called_once_with(
            Key={'userId': user_id, 'serverId': server_id}
        )
    ...
    def test_property_membership_removal_consistency(self, user_id, server_id, email, role, created_by):
        ...
        self.assertIsNone(retrieved_membership, 
                         f"Membership should not exist after deletion: user={user_id}, server={server_id}")
