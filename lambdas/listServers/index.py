...
def fetch_parallel_data(instances):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        instance_types = {
            executor.submit(ec2_client.describe_instance_types, InstanceTypes=[instance["InstanceType"]]): instance
            for instance in instances
        }
        instance_status = {
            executor.submit(ec2_utils.describe_instance_status, instance["InstanceId"]): instance
            for instance in instances
        }
        config_validation = {
            executor.submit(get_server_validation, instance["InstanceId"]): instance for instance in instances
        }
...
def build_server_response(server, instance_types, instance_status, config_validation, user_email):
    ...
    instance_type_future = next(
        future
        for future in instance_types
        if future.result()["InstanceTypes"][0]["InstanceType"] == server["InstanceType"]
    )
    ...
    config_validation_future = next(
        future for future in config_validation if future.result()["instanceId"] == instance_id
    )
    ...
    try:
        ...
        if not user_sub:
            ErrorHandler.log_error(
                "VALIDATION_ERROR", context={"operation": "handler"}, error="No user sub found in token"
            )
        ...
        for server in instances:
            try:
                server_data = build_server_response(
                    server, instance_types, instance_status, config_validation, user_attributes["email"]
                )
        ...
        ErrorHandler.log_error("VALIDATION_ERROR", context={"operation": "handler"}, exception=e, error=str(e))
        return str(e)
    except Exception as e:
        ErrorHandler.log_error("INTERNAL_ERROR", context={"operation": "handler"}, exception=e, error=str(e))
