...
def configure_scheduled_shutdown_event(instance_id, cron_expression, timezone="UTC"):
    ...
    try:
        ...
        if rules.get("Rules"):
            ...
            eventbridge_client.remove_targets(Rule=rule_name, Ids=[f"shutdown-target-{instance_id}"])
    ...
    eventbridge_client.put_rule(Name=rule_name, ScheduleExpression=formatted_schedule, State="ENABLED")
    ...
    target = {
        ...
        "Arn": lambda_arn,
        "Input": json.dumps({"action": "stopServer", "instanceId": instance_id, "source": "scheduled-shutdown"}),
    }
...
def configure_start_event(instance_id, cron_expression, timezone="UTC"):
    ...
    try:
        ...
        if rules.get("Rules"):
            logger.info(f"Removing existing rule {rule_name} before recreating")
            eventbridge_client.remove_targets(Rule=rule_name, Ids=[f"start-target-{instance_id}"])
            eventbridge_client.delete_rule(Name=rule_name)
    except ClientError as e:
        logger.warning(f"Error removing existing rule: {e}")
    ...
    target = {
        ...
        "Arn": lambda_arn,
        "Input": json.dumps({"action": "startServer", "instanceId": instance_id, "source": "scheduled-start"}),
    }
...
def send_to_appsync(action, instance_id, status, message=None, user_email=None):
    ...
    try:
        ...
        request = AWSRequest(
            method="POST", url=endpoint, data=json.dumps(payload), headers={"Content-Type": "application/json"}
        )
...
def _parse_message(message_body):
    ...
    try:
        ...
        ErrorHandler.log_error("VALIDATION_ERROR", context={"operation": "parse_message"}, exception=e, error=str(e))
        return None
...
def _validate_message(message):
    ...
    if "action" not in message:
        ErrorHandler.log_error(
            "VALIDATION_ERROR", context={"operation": "validate_message"}, error="Missing action field"
        )
        return False
...
def handle_server_action(action, instance_id):
    ...
    valid_states = {"start": "stopped", "stop": "running", "restart": "running"}
...
def process_create_server(message):
    ...
    try:
        ...
        logger.info(
            f"Server creation: Processing request - name={server_name}, type={instance_type}, user={user_email}"
        )
        ...
        try:
            ...
            logger.info(
                f"Configuring CPU-based shutdown: instance={instance_id}, threshold={alarm_threshold}, period={alarm_period}"
            )
            ec2_utils.update_alarm(instance_id, "CPUUtilization", alarm_threshold, alarm_period)
            ...
            if utils.is_valid_cron(stop_schedule):
                logger.info(
                    f"Configuring scheduled shutdown: instance={instance_id}, schedule={stop_schedule}, timezone={timezone}"
                )
                configure_scheduled_shutdown_event(instance_id, stop_schedule, timezone)
            ...
            if utils.is_valid_cron(start_schedule):
                logger.info(
                    f"Configuring scheduled start: instance={instance_id}, schedule={start_schedule}, timezone={timezone}"
                )
                configure_start_event(instance_id, start_schedule, timezone)
            ...
            logger.error(
                f"Server creation WARNING: Failed to configure shutdown mechanism - instance={instance_id}, error={str(e)}",
                exc_info=True,
            )
        ...
        try:
            ...
            logger.error(
                f"Server creation WARNING: Failed to store configuration in DynamoDB - instance={instance_id}, error={str(e)}",
                exc_info=True,
            )
        ...
        try:
            ...
            send_to_appsync(
                "createServer", instance_id, "COMPLETED", f"Server {server_name} created successfully", user_email
            )
            logger.info(f"Completion status sent successfully: instance={instance_id}")
        except Exception as e:
            logger.error(
                f"Server creation WARNING: Failed to send status to AppSync - instance={instance_id}, error={str(e)}",
                exc_info=True,
            )

        logger.info(
            f"Server creation SUCCESS: Server {server_name} created successfully with instance ID {instance_id}"
        )
        return True
...
def handle_update_server_name(instance_id, arguments):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Server name update FAILED: Error updating EC2 Name tag - instance={instance_id}, error={str(e)}",
                exc_info=True,
            )
            return False
        ...
        try:
            ...
            logger.error(
                f"Server name update FAILED: Error updating DynamoDB - instance={instance_id}, error={str(e)}",
                exc_info=True,
            )
            return False

        logger.info(
            f"Server name update SUCCESS: Name updated successfully - instance={instance_id}, newName={new_name}"
        )
        return True

    except Exception as e:
        logger.error(
            f"Server name update handler FAILED with exception: instance={instance_id}, error={str(e)}", exc_info=True
        )
        return False
...
def handler(event, context):
    ...
    for idx, record in enumerate(event["Records"]):
        ...
        try:
            ...
            ErrorHandler.log_error(
                "INTERNAL_ERROR", context={"operation": "handler", "message_id": message_id}, exception=e, error=str(e)
            )
