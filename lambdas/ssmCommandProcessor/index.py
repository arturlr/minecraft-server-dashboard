...
def check_instance_ready(instance_id):
    ...
    try:
        ...
        if instance_status == "ok" and system_status == "ok":
            ...
            return {"ready": False, "reason": f"Instance status: {instance_status}, System status: {system_status}"}
...
def send_ssm_command(instance_id, document_name, parameters, comment=None, timeout_seconds=3600):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            logger.info(
                f"SSM command sent successfully: CommandId={command_id}, Instance={instance_id}, Document={document_name}"
            )

            return {"success": True, "commandId": command_id, "message": "Command sent successfully"}
            ...
            if "InvalidInstanceId" in error_msg or "not in a valid state" in error_msg:
                if attempt < max_attempts:
                    ...
                    return {"success": False, "commandId": None, "message": f"Max attempts reached. Error: {error_msg}"}
            else:
                # Non-retryable error
                return {"success": False, "commandId": None, "message": f"Non-retryable error: {error_msg}"}

    return {"success": False, "commandId": None, "message": "Unexpected end of retry loop"}
    ...
    for record in event["Records"]:
        try:
            ...
            if not instance_id or not document_name:
                ...
                results.append({"success": False, "instanceId": instance_id, "message": "Missing required fields"})
                continue

            logger.info(
                f"Processing SSM command: Instance={instance_id}, Document={document_name}, Metadata={metadata}"
            )
            ...
            results.append({"success": False, "message": f"Invalid JSON: {str(e)}"})
        except Exception as e:
