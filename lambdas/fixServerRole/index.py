...
class IamProfile:
    ...
    def disassociate_iam_profile(self):
        ...
        def check_disassociated_status():
            try:
                return (
                    ec2_utils.describe_iam_profile(self.instance_id, "disassociated", self.association_id) is not None
                )
            except Exception as e:
    ...
    def attach_iam_profile(self):
        ...
        try:
            response = self.ec2_client.associate_iam_instance_profile(
                IamInstanceProfile={"Name": ec2_instance_profile_name}, InstanceId=self.instance_id
            )
            ...
            if "UnauthorizedOperation" in error_msg:
                ...
                return {
                    "status": "error",
                    "message": "Unauthorized to attach IAM profile",
...
def check_authorization(event, instance_id, user_attributes):
    ...
    try:
        is_authorized, auth_reason = utl.check_user_authorization(cognito_groups, instance_id, user_email, ec2_utils)
        ...
        logger.error(
            f"Authorization check FAILED: user={user_email}, instance={instance_id}, error={str(e)}", exc_info=True
        )
        raise
