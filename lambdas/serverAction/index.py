...
def send_status_to_appsync(action, instance_id, status, message=None, user_email=None):
    ...
    """
    logger.info(
        f"Action status: action={action}, instance={instance_id}, status={status}, user={user_email}, message={message}"
    )
...
def send_to_queue(action, instance_id, arguments=None, user_email=None):
    ...
    if not is_valid:
        logger.error(
            f"Queue operation FAILED: Message validation failed for action={action}, instance={instance_id}, error={error_message}"
        )
        return utl.response(400, {"err": error_message})

    # Build message with all required fields
    message = {"action": action, "instanceId": instance_id, "timestamp": int(time.time())}
    ...
    logger.info(
        f"Queue operation: Sending message to SQS - action={action}, instance={instance_id}, queue={server_action_queue_url}"
    )

    try:
        # Send message to SQS queue
        response = sqs_client.send_message(QueueUrl=server_action_queue_url, MessageBody=json.dumps(message))
        message_id = response.get("MessageId", "unknown")
        logger.info(
            f"Queue operation SUCCESS: Message sent to SQS - action={action}, instance={instance_id}, messageId={message_id}"
        )
        ...
        logger.error(
            f"Queue operation FAILED with exception: action={action}, instance={instance_id}, error={str(e)}",
            exc_info=True,
        )
    ...
    try:
        ...
        for member in members:
            try:
                ...
                server_users.append(
                    {"id": member["userId"], "email": member["email"], "fullName": full_name, "role": member["role"]}
                )
...
def handle_search_user_by_email(email):
    ...
    try:
        ...
        return {"id": user_info["sub"], "email": user_info["email"], "fullName": user_info["fullName"]}

    except Exception as e:
        logger.error(f"Error searching for user by email {email}: {str(e)}", exc_info=True)
        return utl.response(
            500, {"error": "SEARCH_ERROR", "message": f"An error occurred while searching for user: {str(e)}"}
        )
...
def handle_create_server(input_data, user_attributes):
    ...
    try:
        ...
        if admin_group_name not in cognito_groups:
            logger.warning(
                f"Non-admin user attempted server creation: user={user_attributes.get('email', 'unknown')}, groups={cognito_groups}"
            )
        ...
        if not is_valid:
            logger.error(
                f"Server creation validation failed: user={user_attributes.get('email', 'unknown')}, error={error_message}"
            )
            return utl.response(400, {"err": error_message})
        ...
        logger.info(
            f"Queuing server creation: user={user_attributes.get('email', 'unknown')}, server={input_data['serverName']}"
        )
        ...
        try:
            response = sqs_client.send_message(QueueUrl=server_action_queue_url, MessageBody=json.dumps(message_data))
            message_id = response.get("MessageId", "unknown")
            logger.info(
                f"Server creation queued successfully: messageId={message_id}, server={input_data['serverName']}"
            )
            ...
            logger.error(
                f"Server creation queue FAILED: user={user_attributes.get('email', 'unknown')}, error={str(e)}",
                exc_info=True,
            )
        ...
        logger.error(
            f"Server creation FAILED with exception: user={user_attributes.get('email', 'unknown')}, error={str(e)}",
            exc_info=True,
        )
...
def handle_update_user_role(instance_id, user_id, new_role, requesting_user_attributes):
    ...
    try:
        ...
        logger.info(
            f"Updating user role: user={user_id}, instance={instance_id}, new_role={new_role}, requester={requesting_user_email}"
        )
        ...
        if not (requesting_user_access and requesting_user_access["role"] == "admin") and not is_global_admin:
            ...
            return ErrorHandler.create_error_response(
                403, "INSUFFICIENT_PERMISSIONS", error="Insufficient permissions. Admin role required."
            )

        # Step 2: Prevent users from modifying their own admin role (lockout protection)
        if (
            user_id == requesting_user_sub
            and requesting_user_access
            and requesting_user_access["role"] == "admin"
            and new_role != "admin"
        ):
            logger.warning(f"Self-admin-role modification blocked: user={requesting_user_email}")
            return ErrorHandler.create_error_response(
                403, "SELF_ADMIN_ROLE_PROTECTION", error="Cannot modify your own admin role to prevent lockout"
            )
        ...
        try:
            ...
            if not current_membership:
                return ErrorHandler.create_error_response(
                    404, "MEMBERSHIP_NOT_FOUND", error="User membership not found"
                )
            ...
            if "not found" in str(e):
                ...
                return ErrorHandler.create_error_response(
                    404, "USER_MEMBERSHIP_NOT_FOUND", error=f"User membership not found for server {instance_id}"
                )
            else:
        ...
        return ErrorHandler.create_error_response(
            500, "INTERNAL_ERROR", error=f"An unexpected error occurred: {str(e)}"
        )
...
def handle_remove_user_from_server(instance_id, user_id, requesting_user_attributes):
    ...
    try:
        ...
        logger.info(
            f"Removing user from server: user={user_id}, instance={instance_id}, requester={requesting_user_email}"
        )
        ...
        logger.error(
            f"Error removing user from server: user={user_id}, instance={instance_id}, error={str(e)}", exc_info=True
        )
        return ErrorHandler.create_error_response(
            500, "INTERNAL_ERROR", error=f"An unexpected error occurred: {str(e)}"
        )
...
def route_instance_operation(event, instance_id, user_attributes):
    ...
    try:
        is_authorized, user_role, auth_reason = core_dyn.check_user_authorization(
            user_attributes["sub"], instance_id, required_permission
        )
        if not is_authorized:
            logger.error(
                f"User {user_attributes['email']} not authorized for {field_name} on instance {instance_id}: {auth_reason}"
            )
        ...
        logger.error(
            f"Authorization check FAILED: user={user_attributes.get('email', 'unknown')}, instance={instance_id}, error={str(e)}",
            exc_info=True,
        )
