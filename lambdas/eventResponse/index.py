...
def send_to_appsync(payload):
    ...
    try:
        ...
        response = httpxClient.post(endpoint, headers=headers, json=payload)
        logger.info(response.json())
    except Exception as e:
        ErrorHandler.log_error("NETWORK_ERROR", context={"operation": "send_to_appsync"}, exception=e, error=str(e))
...
def schedule_event_response():
    ...
    for instance in instances_running["Instances"]:
        instance_info = {
            ...
            "memStats": utl.get_metrics_data(
                instance["InstanceId"], "CWAgent", "mem_used_percent", "Percent", "Average", dt_start_time, dt_now, 60
            ),
            "cpuStats": utl.get_metrics_data(
                instance["InstanceId"], "CWAgent", "cpu_usage_active", "Percent", "Average", dt_start_time, dt_now, 60
            ),
            "networkStats": utl.get_metrics_data(
                instance["InstanceId"],
                "MinecraftDashboard",
                "transmit_bandwidth",
                "Bytes/Second",
            ...
            "activeUsers": utl.get_metrics_data(
                instance["InstanceId"],
                "MinecraftDashboard",
                "user_count",
                "Count",
    ...
    try:
        ...
        if "detail-type" not in event:
            ErrorHandler.log_error(
                "VALIDATION_ERROR", context={"operation": "handler"}, error="No detail-type found in event"
            )
